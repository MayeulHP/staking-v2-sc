...
pub trait StakingV2ScContract: common::CommonModule + utils::UtilsModule {
    #[init]
    fn init(
        &self,
    ...
    #[view(stakedIter)]
    #[storage_mapper("staked_iter")]
    fn staked_iter(
        &self,
        user: &ManagedAddress,
        token_id: &TokenIdentifier,
    ) -> SetMapper<(u64, BigUint<Self::Api>)>;
    ...
    fn gns_reward(
        &self,
        episode_rewards: BigUint,
        building_rarity: BuildingRarity,
    ) -> BigUint<Self::Api> {
    ...
    fn deposit_ecity(&self) {
        ...
        require!(
            caller == self.router_address().get(),
            "Only the router can deposit"
        );
        let payment = self.call_value().single_esdt();
        require!(
            payment.token_identifier == self.ecity_tokenid().get_token_id(),
            "Only ECITY can be deposited"
        );
        ...
        self.episodes_rewards(self.current_episode().get())
            .set(payment.amount);
        self.episodes_timestamps(self.current_episode().get())
    ...
    fn add_ecity(&self) {
        ...
        self.episodes_rewards(current_episode)
            .set(self.episodes_rewards(current_episode).get() + payment.amount);
    }
    ...
    fn stake(&self) {
        ...
        for payment in self.call_value().all_esdt_transfers().iter() {
            require!(
                self.collections().contains(&payment.token_identifier),
                "Token not supported"
            );
            ...
            if !self
                .staked_iter(&caller, &payment.token_identifier)
                .iter()
                .any(|(nonce, _)| nonce == payment.token_nonce)
            {
            ...
            self.nb_staked(&caller)
                .set(self.nb_staked(&caller).get() + u64::from(payment_amount_u64));

            sc_print!(
                "Staked {} of token {} with nonce {}",
                payment.amount,
    ...
    fn unstake_single(&self, token_id: TokenIdentifier, nonce: u64) {
        ...
        require!(
            self.staked(&caller, &token_id, &nonce).get() > BigUint::from(0u8),
            "Nothing to unstake"
        );
        ...
        self.staked(&caller, &token_id, &nonce)
            .set(self.staked(&caller, &token_id, &nonce).get() - BigUint::from(1u8));

        if self.staked(&caller, &token_id, &nonce).get() == BigUint::from(0u8) {
            // If the amount is zero, remove the nonce from the staked_iter
            self.staked_iter(&caller, &token_id)
                .remove(&(nonce, BigUint::from(1u8)));
        } else {
            // Remove the old amount and add the new one
            self.staked_iter(&caller, &token_id).remove(&(
                nonce,
                self.staked(&caller, &token_id, &nonce).get() + BigUint::from(1u8),
            ));
            self.staked_iter(&caller, &token_id)
                .insert((nonce, self.staked(&caller, &token_id, &nonce).get()));
        }

        self.nb_staked(&caller)
            .set(self.nb_staked(&caller).get() - 1);
        ...
        self.send()
            .direct_esdt(&caller, &token_id, nonce, &BigUint::from(1u8));
    }
    ...
    fn claim_ecity(&self, episode: u64, addr: &ManagedAddress) {
        ...
        if self.episodes_rewards(episode).get() <= BigUint::zero()
            || self.current_episode().get() < episode
        {
            return;
        }
        ...
        self.collections().iter().for_each(|token_id| {
            self.staked_iter(&addr, &token_id.clone())
                .iter()
                .for_each(|(nonce, amount)| {
        ...
        self.claimed_per_episode(episode)
            .set(self.claimed_per_episode(episode).get() + &to_be_sent);
        ...
        if episode == self.current_episode().get() {
            ...
            self.staked_time(&addr)
                .set(self.episodes_timestamps(episode).get() + 1209600u64);
        }

        if to_be_sent > BigUint::from(0u8) {
            self.send()
                .direct_esdt(&addr, &self.ecity_tokenid().get_token_id(), 0, &to_be_sent);
        }
    }
    ...
    fn claim(&self) {
        ...
        self.last_episode_claimed(&caller)
            .set(self.current_episode().get());
        sc_print!(
            "Last episode claimed: {}",
            self.last_episode_claimed(&caller).get()
        );
    }
    ...
    fn fake_claim(&self, addr: &ManagedAddress) -> BigUint<Self::Api> {
        ...
        for episode in last_episode_claimed..(self.current_episode().get() + 1u64) {
            ...
            to_be_sent = to_be_sent * staked_length / BigUint::from(1209600u64);
    ...
    fn claim_unclaimable(&self, episode: u64) {
        ...
        require!(
            self.episodes_rewards(episode).get() > BigUint::from(0u8),
            "Nothing to claim"
        );
        require!(
            self.current_episode().get() > episode,
        ...
        self.send().direct_esdt(
            &caller,
            &self.ecity_tokenid().get_token_id(),
            0,
            &to_be_sent,
        );
    }
